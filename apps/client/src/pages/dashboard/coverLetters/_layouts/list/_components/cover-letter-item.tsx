...
export const CoverLetterListItem = ({ resume }: Props) => {
  const navigate = useNavigate();
  const { open } = useDialog<ResumeDto>("coverLetter");
  const { open: lockOpen } = useDialog<ResumeDto>("lockCL");
  ...
  const onUpdate = () => {
    open("update", { id: "coverLetter", item: resume });
  };

  const onDuplicate = () => {
    open("duplicate", { id: "coverLetter", item: resume });
  };

  const onLockChange = () => {
    lockOpen(resume.locked ? "update" : "create", { id: "lockCL", item: resume });
  };

  const onDelete = () => {
    open("delete", { id: "coverLetter", item: resume });
  };
