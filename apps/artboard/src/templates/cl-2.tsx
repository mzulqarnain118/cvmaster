          ...
          <div>
            ...
            {basics.phone && (
              <div className="flex items-center gap-x-1.5">
                ...
                <a className="break-words max-w-[200px]" href={`tel:${basics.phone}`} target="_blank" rel="noreferrer">
                  {basics.phone}
                </a>
              </div>
            )}
            {basics.email && (
              <div className="flex items-center gap-x-1.5">
                ...
                <a className="break-words max-w-[200px]" href={`mailto:${basics.email}`} target="_blank" rel="noreferrer">
                  {basics.email}
                </a>
              </div>
            )}
            ...
            {basics.customFields.map((item) => (
              <div key={item.id} className="flex items-center gap-x-1.5">
                ...
                {isUrl(item.value) ? (
                  <a className="break-words max-w-[200px]" href={item.value} target="_blank" rel="noreferrer noopener nofollow">
                    {item.name || item.value}
                  </a>
                ) : (
                  <span className="break-words max-w-[200px]">{[item.name, item.value].filter(Boolean).join(": ")}</span>
                )}
              </div>
            ))}
          </div>
      ...
      <section id="recipient" className="space-y-2">
        ...
        <main className={cn("relative space-y-2", "border-l border-primary pl-4")}>
          ...
          <div className="space-y-1">
            ...
            {recipient?.email && (
              <a className="break-words max-w-[200px]" href={`mailto:${recipient?.email}`} target="_blank" rel="noreferrer">
                {recipient?.email}
              </a>
            )}
            {recipient?.subject && <div className="break-words max-w-[200px]">{recipient?.subject}</div>}
            {recipient?.greeting && <div className="break-words max-w-[200px]">{recipient?.greeting}</div>}
          </div>
        </main>
      </section>
    );
